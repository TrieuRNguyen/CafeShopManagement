...
namespace CafeShopManagementSystem.frmCashierMainFromforder
{
    public partial class frmCashierOrder : UserControl
    {
        ...
        {
            CashierOrderFromProdData allProds = new CashierOrderFromProdData();

            List<CashierOrderFromProdData> listData = allProds.availableProductsData();
                        ...
                        using (SqlCommand cmd = new SqlCommand(selectData, connect))
                        {
                            ...
                            using (SqlDataReader reader = cmd.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    ...
                                    lb_ProductsName.Text = prodName;
                                    lb_Price.Text = prodPrice;
        ...
        private void cashierOrderForm_addBtn_Click(object sender, EventArgs e)
        {
            ...
            if (cashierOrderForm_type.SelectedIndex == -1 || cashierOrderForm_productID.SelectedIndex == -1
                || lb_ProductsName.Text == "" || cashierOrderForm_quantity.Value == 0
                || lb_Price.Text == "")
            {
                ...
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(insertOrder, connect))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@prodName", lb_ProductsName.Text);
        ...
        private void cashierOrderForm_receiptBtn_Click(object sender, EventArgs e)
        {
            ...
            // Gán tài liệu cần in cho dialog
            printPreviewDialog1.Document = printDocument1;

            try
            {
                printPreviewDialog1.ShowDialog(); // Hiển thị hộp thoại xem trước
            }
            catch (Exception ex)
            {
        ...
        private void printDocument1_PrintPage_1(object sender, PrintPageEventArgs e)
        {
            ...
            string headerText = "Cafe Shop Nhom 12";
